  ...
  <div class="confirm-container">
    <div class="confirm-modal">
      ...
      <div class="modal--content">
        <div>{{ this.$EmulationResources["VN"].AgreeToChange }}</div>
        <div class="cf-modal__div--button">
          <button class="no-cf" @click="closeCancelModal">
            {{ this.$EmulationResources["VN"].Cancel }}
          </button>
          <button class="no-cf" @click="closeModal">
            {{ this.$EmulationResources["VN"].Not }}
          </button>
          <button class="cf" @click="saveChangeData">
            {{ this.$EmulationResources["VN"].UpdateEmulation }}
          </button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    /**
     * Hàm gửi emit đóng form sửa và modal xác nhận
     * CreatedBy: NVA - MF1618
     */
    closeCancelModal() {
      this.$emitter.emit("closeAllModal");
    },

    /**
     * Hàm gửi emit đóng modal xác nhận
     * CreatedBy: NVA - MF1618
     */
    closeModal() {
      this.$emitter.emit("closeCancelModal");
    },

    /**
     * Hàm gửi emit cập nhật bản ghi, đóng form sửa và modal xác nhận
     * CreatedBy: NVA - MF1618
     */
    saveChangeData() {
